...
class cleanTweet:
    ...
    def replaceAtMention(self):
        ...
        for match in matches:
            replacement_list=replacement.append((match.group(2)).upper())              #making another list corresponding to the word after @ 
                                                                                       #upper case (@rohan-->ROHAN)
        for match,replacement in zip(matches,replacement_list):
            re.sub(match, '*{}*'.format(replacement), self.tweet)             #looping through the combined list made using zip function
    ...
    def seperateCojoinedWords(self):
        ...
        for pattern in patterns:
            re.sub(pattern,' {}'.format(pattern),self.tweet)      #puts a space to seperate when it finds a cojoined capital letter
